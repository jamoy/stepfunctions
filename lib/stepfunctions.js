...
class StepFunction extends EventEmitter {
  ...
  inputPath(state, input) {
    ...
    if (template) {
      ...
      // Evaluate the template against the input captured before any retries so
      // a retried Task does not re-apply its Parameters to an already
      // transformed input (which would compound/recurse the payload on every
      // attempt).
  ...
  compare(choice, input) {
    ...
    // Data-test operators must run even when the field is absent, e.g.
    ...
    if (choice.IsString !== undefined) {
      return (typeof variable === 'string') === choice.IsString;
    }
    if (choice.IsNumeric !== undefined) {
      return (typeof variable === 'number') === choice.IsNumeric;
    }
    if (choice.IsBoolean !== undefined) {
      return (typeof variable === 'boolean') === choice.IsBoolean;
    }
    if (choice.IsTimestamp !== undefined) {
      return this.isTimestamp(variable) === choice.IsTimestamp;
    }
