...
class StepFunction extends EventEmitter {
   ...
   * @param input
   */
  transformParameters(parameters, input) {
    const newInput = {};
    ...
    Object.keys(parameters)
      ...
      .map((parameter) => {
        if (typeof parameters[parameter] === 'object') {
          newInput[parameter.replace('.$', '')] = this.transformParameters(parameters[parameter], input);
        } else if (
          parameter.includes('.$') &&
          parameters[parameter].includes('States.')
        ) {
          // update from august 2020
          if (parameters[parameter].startsWith('States.Format(')) {
            newInput[parameter.replace('.$', '')] = this.intrinsicFormat(parameters[parameter], input);
          } else if (parameters[parameter].startsWith('States.StringToJson(')) {
            newInput[parameter.replace('.$', '')] = this.intrinsicStringToJson(parameters[parameter], input);
          } else if (parameters[parameter].startsWith('States.JsonToString(')) {
            newInput[parameter.replace('.$', '')] = this.intrinsicJsonToString(parameters[parameter], input);
          } else if (parameters[parameter].startsWith('States.Array(')) {
            newInput[parameter.replace('.$', '')] = this.intrinsicArray(parameters[parameter], input);
          }
        } else if (
          parameter.includes('.$') &&
          !parameters[parameter].includes('$$.')
        ) {
          newInput[parameter.replace('.$', '')] = jsonpath
            .query(input, parameters[parameter])
            .shift();
        } else if (
          parameter.includes('.$') &&
          parameters[parameter].includes('$$.')
          ...
          newInput[parameter.replace('.$', '')] = jsonpath
            .query(context, parameters[parameter].replace('$$.', '$.'))
            .shift();
        } else {
          newInput[parameter] = parameters[parameter];
        }
      });
    return newInput;
  }
  ...
  inputPath(state, input) {
    ...
    if (state.Parameters) {
      input = this.transformParameters(state.Parameters, input);
    }
    return input;
  }
  ...
  intrinsicStringToJson(template, input) {
    // console.log(template, input);
  }

  intrinsicJsonToString(template, input) {
    // console.log(template, input);
  }

  intrinsicArray(template, input) {
    // console.log(template, input);
  }

  buildFromJsonPath(obj, path) {
    // TODO: Support $..key (which is a recursive assignment)
    // TODO: Support $.key[0].test
    console.log(jsonpath.parse(path));
    let root = {};
    jsonpath.parse(path).map((action) => {
      if (action.expression.type === 'identifier' && action.scope === 'child' && !obj[action.expression.value]) {
        root[action.expression.value] = {};
      }
    });
    return obj;
  }
  ...
  outputPath(state, input, output) {
    ...
    if (!state.ResultPath?.startsWith('$$') && state.ResultPath) {
      ...
      // const existing = jsonpath.value(input, state.ResultPath, output || originalValue);
      console.log(this.buildFromJsonPath(input, state.ResultPath));
      // console.log(input, state.ResultPath, jsonpath.apply(input, state.ResultPath, () => input).shift());
      // if (existing) {
      //   // override
      // } else {
      //   // append
    ...
    if (state.OutputPath) {
      const newOutput = this.buildFromJsonPath({}, state.OutputPath);
      output = jsonpath.apply(newOutput, state.OutputPath, () => input).shift();
      return output.value;
    }
    ...
    if (state.ResultSelector) {
      // update from august 2020
      // The value of "ResultSelector" MUST be a Payload Template, whose input is the result, and whose payload replaces and becomes the effective result.
    }
    console.log(output);
    return output;
  }
