...
class StepFunction extends EventEmitter {
        ...
        .map((parameter) => {
          if (typeof state.Parameters[parameter] === 'object') {
            ...
            if (state.Parameters[parameter].startsWith('States.Format(')) {
              state.Parameters[parameter] = this.intrinsicFormat(state.Parameters[parameter], context, input)
            } else if (state.Parameters[parameter].startsWith('States.StringToJson(')) {
              state.Parameters[parameter] = this.intrinsicStringToJson(state.Parameters[parameter], context, input)
            } else if (state.Parameters[parameter].startsWith('States.JsonToString(')) {
              state.Parameters[parameter] = this.intrinsicJsonToString(state.Parameters[parameter], context, input)
            } else if (state.Parameters[parameter].startsWith('States.Array(')) {
              state.Parameters[parameter] = this.intrinsicArray(state.Parameters[parameter], context, input)
            }
   ...
   * @param {any} input
   */
  intrinsicFormat(template, context, input) {
    const formatted = '';
    return formatted;
  }
