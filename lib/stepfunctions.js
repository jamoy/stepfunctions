...
class StepFunction extends EventEmitter {
  ...
  async startExecution(input, opts = {}) {
    ...
    try {
      ...
      if (!err.caught) { // only throw if it doesnt have a Catch or a Retry handler
  ...
  outputPath(state, input, output) {
    if (state.ResultPath !== undefined && (typeof input !== 'object' && !Array.isArray(input))) {
  ...
  async finally(states, input, originalState, error) {
    for (let state of states) {
      const errors = state.ErrorEquals.filter(err => {
        ...
        if (err === ErrorState.Runtime && (error.original && !!error.original.toString().match(/States.Runtime|Eval|Type|Syntax|URI|Range|Error|parse|Parsing|Unexpected|token|Reference|undefined|of null|read property|JSON/g))) {
          return true;
        }
        return false;
      });
