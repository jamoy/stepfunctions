    ...
    try {
      ...
      const compare = (choice) => {
        ...
        if (typeof variable === 'number') {
          ...
          if (variable <= choice.NumeriLessThanEquals) {
            return selectChoice(choice);
          }
        }

        // use timestamp
        if (new Date(variable).getTime() > 0) {
      ...
      // TODO: explicit for now, because it's hard to visualize right now
      for (let choice of state.Choices) {
        if (choice.NOT) {
          return compare(choice.NOT);
        }
