...
class StepFunction extends EventEmitter {
  ...
  inputPath(state, input) {
    ...
    if (state.Parameters) {
      ...
      // Evaluate Parameters against the input captured before any retries so a
      // retried Task does not re-apply its Parameters to an already-transformed
      // input (which would compound/recurse the payload on every attempt).
