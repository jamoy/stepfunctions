    ...
    Object.keys(parameters)
      ...
      .map((parameter) => {
        if (typeof parameters[parameter] === 'object') {
          newInput[parameter] = {};
          Object.keys(parameters[parameter]).map((key) => {
            if (typeof parameters[parameter][key] === 'object') {
              newInput[parameter][key.replace('.$', '')] = this.transformParameters(parameters[parameter][key], input);
            } else if (
              key.includes('.$') &&
              !parameters[parameter][key].includes('$$.')
            ) {
              newInput[parameter][key.replace('.$', '')] = jsonpath
                .query(input, parameters[parameter][key])
                .shift();
            } else if (
              key.includes('.$') &&
              parameters[parameter][key].includes('$$.')
              ...
              newInput[parameter][key.replace('.$', '')] = jsonpath
                .query(
                  context,
                  parameters[parameter][key].replace('$$.', '$.'),
                )
                .shift();
            } else {
              newInput[parameter][key] = parameters[parameter][key];
            }
          });
        } else if (
          parameter.includes('.$') &&
