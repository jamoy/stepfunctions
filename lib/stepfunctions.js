...
class StepFunction extends EventEmitter {
  ...
  async map(state, task, input, states) {
    ...
    try {
      ...
      // threshold. Each iteration runs in its own variable scope: it may read
      // outer variables but its assignments do not leak back to the parent.
      const parentVariables = { ...this.variables };
      ...
      this.variables = parentVariables;
  ...
  async parallel(state, task, input, states) {
    ...
    try {
      let output = [];
      // Branches run in their own variable scope: they may read outer variables
      // but their assignments do not leak back to the parent.
      const parentVariables = { ...this.variables };
  ...
  refresh() {
    ...
    this.transitions = 0;
    this.steps = [];
    // Assigned workflow variables (the `Assign` field / `$var` references).
    this.variables = {};
