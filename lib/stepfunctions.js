...
class StepFunction extends EventEmitter {
  ...
  async map(state, task, input, states) {
    ...
    try {
      ...
      // Phase 1 (synchronous): shape each item via ItemSelector/Parameters,
      // capturing its Map.Item context before the (possibly concurrent) runs.
      const selected = items.map((item, index) => {
        ...
        return this.inputPath(state, item);
      });
      ...
      const finalOutput = this.resultSelector(state, output);
      if (state.Next) {
