...
ErrorState.QueryEvaluationError = 'States.QueryEvaluationError';
...
class StepFunction extends EventEmitter {
   ...
   * variables
   *
   * The workflow variables for the currently executing scope (the active Map
   * iteration / Parallel branch, or the root scope at the top level).
   *
   * @access private
   */
  get variables() {
    return this.variableStore.getStore() ?? this._rootVariables;
  }
  ...
  evaluatePayloadValue(expr, input, context) {
    ...
    // `$varName` (a letter immediately after `$`) is a workflow variable, as
    // opposed to `$.path` (input) or `$$.path` (context).
   ...
   * isJSONata
   *
   * Whether a state is evaluated with the JSONata query language (a per-state
   * `QueryLanguage` wins over the machine default).
   *
   * @access private
   */
  isJSONata(state) {
    return (state.QueryLanguage || this.queryLanguage) === 'JSONata';
  }

  /**
   * buildStates
   *
   * Constructs the reserved `$states` object exposed to JSONata expressions. The
   * context is snapshotted so concurrent Map iterations each see their own
   * `$states.context.Map.Item`.
   *
   * @access private
   */
  buildStates(input, extra = {}) {
   ...
   * evaluateJSONata
   *
   * Recursively evaluates a JSONata value: any string wrapped in `{% %}` is
   * evaluated as a JSONata expression (with `$states` and the in-scope variables
   * bound); objects/arrays are walked; every other value is a literal.
   *
   * @access private
   */
  async evaluateJSONata(value, statesObj) {
    if (typeof value === 'string') {
      ...
      let result;
      try {
        result = await jsonata(expression).evaluate(null, {
          // `$states` is reserved, so it must win over any like-named variable.
          ...this.variables,
          states: statesObj,
        });
      } catch (err) {
        throw new ErrorState(
          ErrorState.QueryEvaluationError,
          `JSONata evaluation failed: ${err.message}`,
        );
      }
      if (result === undefined) {
        throw new ErrorState(
          ErrorState.QueryEvaluationError,
          `JSONata expression returned no result: ${expression}`,
        );
      }
      return result;
    }
    if (Array.isArray(value)) {
      return Promise.all(
        value.map((item) => this.evaluateJSONata(item, statesObj)),
      );
    }
    if (value && typeof value === 'object') {
      const result = {};
      for (const key of Object.keys(value)) {
        result[key] = await this.evaluateJSONata(value[key], statesObj);
      }
      return result;
    }
    return value;
   ...
   * resolveVariable
   *
   * Resolves a `$varName` / `$varName.path` reference (JSONPath mode) against
   * the assigned workflow variables.
   *
   * @access private
   */
  resolveVariable(expr) {
    ...
    const value = this.variables[match[1]];
    if (!match[2]) {
      return value;
    }
    return jsonpath
      .query(value === undefined ? {} : value, '$' + match[2])
      .shift();
  }
   ...
   * applyAssign
   *
   * Evaluates a state's `Assign` block (in whichever query language) and merges
   * the results into the workflow variables. All right-hand sides are evaluated
   * against the pre-assignment variable values, then merged atomically.
   *
   * @access private
   * @param {Object} state
   * @param {Object} statesObj the `$states` object (JSONata mode)
   * @param {any} input the data `$` resolves against (JSONPath mode)
   */
  async applyAssign(state, statesObj, input) {
    if (!state.Assign) {
      return;
    }
    const assigned = this.isJSONata(state)
      ? await this.evaluateJSONata(state.Assign, statesObj)
      : this.evaluatePayloadTemplate(state.Assign, input, this.getContext());
    Object.assign(this.variables, assigned);
  }
  ...
  async task(state, task, input, states) {
    const jsonataMode = this.isJSONata(state);
        ...
        if (jsonataMode) {
          stepInput =
            state.Arguments !== undefined
              ? await this.evaluateJSONata(
                  state.Arguments,
                  this.buildStates(input),
                )
              : input;
        } else {
          stepInput = this.inputPath(state, input);
        }
    ...
    if (jsonataMode) {
      // Output/Assign both see the raw result; Output is computed before the
      // assignment is merged so it cannot observe this state's own variables.
      const statesObj = this.buildStates(input, { result: output });
      const transformed =
        state.Output !== undefined
          ? await this.evaluateJSONata(state.Output, statesObj)
          : output;
      await this.applyAssign(state, statesObj, null);
      output = transformed;
    } else {
      // ResultSelector runs against the raw result, before ResultPath/OutputPath.
      output = this.resultSelector(state, output);
      await this.applyAssign(state, null, output);
    }
    if (state.Next) {
  ...
  async map(state, task, input, states) {
    ...
    try {
      ...
      if (jsonataMode) {
        items =
          state.Items !== undefined
            ? await this.evaluateJSONata(state.Items, this.buildStates(input))
            : input;
      } else {
        items = state.ItemsPath
          ? jsonpath.query(input, state.ItemsPath).shift()
          : input;
      }
      if (!items) {
        items = [];
      }
      this.transition('MapStateStarted', { task, length: items.length });

      // Phase 1: shape each item via ItemSelector/Parameters. The per-item
      // context is snapshotted synchronously (in buildStates / inputPath)
      // before any await, so concurrent iterations don't race on it.
      const selected = await Promise.all(
        items.map((item, index) => {
          this.transition('MapIterationStarted', { task, index, input: item });
          this.createContext(state, task, item, index);
          if (jsonataMode) {
            return state.ItemSelector !== undefined
              ? this.evaluateJSONata(state.ItemSelector, this.buildStates(item))
              : Promise.resolve(item);
          }
          return Promise.resolve(this.inputPath(state, item));
        }),
      );

      // Phase 2: optional ItemBatcher grouping.
      const iterations = state.ItemBatcher
        ? this.batchItems(state, selected, input)
        : selected;

      // Phase 3: run honoring MaxConcurrency and the tolerated-failure
      // threshold. Each iteration runs in its own variable scope.
      const output = await this.runMapIterations(
        state,
        task,
        processor,
        iterations,
      ...
      if (jsonataMode) {
        ...
        finalOutput = this.resultSelector(state, output);
        await this.applyAssign(state, null, output);
      }
      if (state.Next) {
        return this.step(state.Next, finalOutput, states);
      }
      return finalOutput;
    } catch (err) {
      if (err.state === 'Internal.Aborted') {
   ...
   * runMapIterations
   *
   * Runs the Map iterations in batches that respect MaxConcurrency (0 or omitted
   * means unlimited; otherwise the per-execution `maxConcurrency` default), and
   * the ToleratedFailureCount / ToleratedFailurePercentage threshold. Output
   * order matches input order; tolerated failures are dropped from the result.
   *
   * @access private
   */
  async runMapIterations(state, task, processor, iterations) {
    let concurrency;
    if (state.MaxConcurrency === undefined) {
      concurrency = this.runtime.maxConcurrency;
    } else if (state.MaxConcurrency === 0) {
      concurrency = iterations.length;
    } else {
      concurrency = state.MaxConcurrency;
    }
    const batchSize = Math.max(1, concurrency || iterations.length || 1);
    const tolerated = this.toleratedFailures(state, iterations.length);
    const output = new Array(iterations.length);
    const succeeded = new Array(iterations.length).fill(false);
    let failures = 0;
    let firstError;

    for (
      let start = 0;
      start < iterations.length && failures <= tolerated;
      start += batchSize
    ) {
      await Promise.all(
        iterations.slice(start, start + batchSize).map(async (item, offset) => {
          const index = start + offset;
          try {
            // Fork the current scope so each iteration reads outer variables but
            // its own assignments stay isolated from siblings and the parent.
            const result = await this.variableStore.run(
              { ...this.variables },
              () => this.step(processor.StartAt, item, processor.States),
            );
            output[index] = this.outputPath(state, item, result);
            ...
            this.transition('MapIterationFailed', { task, index, error: err });
          }
        }),
      );
    }
    ...
    // Preserve order; drop any tolerated failures.
    return output.filter((_, index) => succeeded[index]);
  }

  /**
   * batchItems
   *
   * Groups Map items into batches per ItemBatcher. Each batch becomes an
   * iteration input of the shape `{ Items: [...], BatchInput?: {...} }`.
   *
   * @access private
   */
  batchItems(state, selected, mapInput) {
    const batcher = state.ItemBatcher;
    const size = batcher.MaxItemsPerBatch || selected.length || 1;
    const batchInput = batcher.BatchInput
      ? this.evaluatePayloadTemplate(
          batcher.BatchInput,
          mapInput,
          this.getContext(),
        )
    ...
    for (let i = 0; i < selected.length; i += size) {
      const items = selected.slice(i, i + size);
      batches.push(
        batchInput === undefined
          ? { Items: items }
          : { Items: items, BatchInput: batchInput },
      );
    }
    return batches;
  }
   ...
   * toleratedFailures
   *
   * Resolves how many iteration failures the Map tolerates before failing.
   * Defaults to 0 (any failure fails the Map).
   *
   * @access private
   */
  toleratedFailures(state, total) {
    if (state.ToleratedFailureCount !== undefined) {
      return state.ToleratedFailureCount;
    }
    if (state.ToleratedFailurePercentage !== undefined) {
      return Math.floor((state.ToleratedFailurePercentage / 100) * total);
    }
    return 0;
  }
      ...
      while (branches.length) {
        const result = await Promise.all(
          branches.splice(0, this.runtime.maxConcurrency).map((branch) => {
            ...
            input = jsonataMode ? input : this.inputPath(branch, input);
            this.transition('ParallelStateStarted', {
              task: branch.StartAt,
              input,
            });
            // Each branch runs in its own forked variable scope, so a branch's
            // assignments stay isolated from its siblings and the parent.
            return this.variableStore.run({ ...this.variables }, () =>
              this.step(branch.StartAt, input, branch.States),
            );
          }),
        );
        output = jsonataMode
          ? [...output, ...result]
          : this.outputPath(state, input, [...output, ...result]);
      }
      this.transition('ParallelStateSucceeded', { task });
      this.transition('ParallelStateExited', { task, output });
      if (jsonataMode) {
        const statesObj = this.buildStates(originalInput, { result: output });
        const transformed =
          state.Output !== undefined
            ? await this.evaluateJSONata(state.Output, statesObj)
            : output;
  ...
  async choice(state, task, input, states) {
    ...
    try {
      ...
      for (let choice of state.Choices) {
        const matched = jsonataMode
          ? (await this.evaluateJSONata(
              choice.Condition,
              this.buildStates(input),
            )) === true
          : this.compare(choice, input) === true;
        if (matched) {
  ...
  async pass(state, task, input, states) {
    ...
    let output = input;
    if (this.isJSONata(state)) {
      const statesObj = this.buildStates(input);
      if (state.Output !== undefined) {
        output = await this.evaluateJSONata(state.Output, statesObj);
      }
      await this.applyAssign(state, statesObj, input);
    } else {
      await this.applyAssign(state, null, output);
    }
    this.transition('PassStateExited', { task, output });
    if (state.Next) {
      return this.step(state.Next, output, states);
    }
    return output;
  ...
  refresh() {
    ...
    this.transitions = 0;
    this.steps = [];
    // Assigned workflow variables (the `Assign` field / `$var` references) for
    // the root scope. Child scopes are forked via `variableStore`.
    this._rootVariables = {};
