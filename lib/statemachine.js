...
class StateMachine {
  constructor(sm, opts) {
    const id = opts.use;
    const { isValid, errorsText } = aslValidator(sm[id].definition);
    if (!isValid) {
    ...
    this.name = sm[id].name;
    this.definition = sm[id].definition;
  ...
  async startExecution(input) {
    ...
    try {
      const output = await this.step(this.definition.StartAt, input, this.definition.States);
